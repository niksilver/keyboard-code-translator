...
function mod_keyboard_process(type, code, value)
  state.menu.values = {type, code, value, nil, nil, nil}
  if code ~= nil then
    local name = keyboard.codes[code]
    state.menu.values[4] = name

    local want_name = state.overrides[code]
    state.menu.values[5] = want_name

    code = want_name and state.revcodes[want_name] or code
    state.menu.values[6] = code
  end
...
function mod_redraw()
  ...
  if state.menu.values[4] then
    name = " (" .. tostring(state.menu.values[4]) .. ")"
  end

  local trans = ""
  if state.menu.values[5] then
      trans = " -> " .. tostring(state.menu.values[6]) ..
      " (" .. tostring(state.menu.values[5]) .. ")"
      name = ""
  end
